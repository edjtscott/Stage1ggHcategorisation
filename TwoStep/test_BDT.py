...
allVars = ['dipho_leadIDMVA','dipho_subleadIDMVA','dipho_lead_ptoM','dipho_sublead_ptoM','dipho_mva', 'dijet_leadEta','dijet_subleadEta','dijet_LeadJPt','dijet_SubJPt','dijet_abs_dEta', 'dijet_Mjj', 'dijet_nj', 'cosThetaStar', 'dipho_cosphi', 'vtxprob','sigmarv','sigmawv','weight', 'HTXSstage0cat', 'HTXSstage1_1_cat','dipho_mass','cos_dijet_dipho_dphi','dipho_leadEta','dipho_subleadEta','dijet_Zep']
...
#add the target variable and the equalised weight
  ...
  sigSumW = np.sum(trainTotal[trainTotal.truthVhHad>0.5]['weight'].values)#summing weights of vh hadronic events (selected by gen HTXS category)
  bkgSumW = np.sum(trainTotal[trainTotal.truthVhHad<0.5]['weight'].values)#summing weights of non-vh hadronic events (selected by gen HTXS stage 0 category
...
vhHadVars = ['dipho_lead_ptoM','dipho_sublead_ptoM','dipho_mva','dijet_leadEta','dijet_subleadEta','dijet_LeadJPt','dijet_SubJPt','dijet_abs_dEta','dijet_Mjj','dijet_nj', 'cosThetaStar', 'cos_dijet_dipho_dphi','dijet_dipho_dEta','dijet_centrality_gg']
...
vhHadModel = xg.train(trainParams, trainMatrix,150,watchlist)
print 'done'
#print progress
...
plt.hist(vhHadPredYtrain[(vhHadTrainY>0.5)], bins=50,weights=vhHadTrainTW[(vhHadTrainY>0.5)],range=[0,1], alpha=0.5, histtype='stepfilled',normed=1, color='red',label='EW qqH')
...
plt.hist(vhHadPredYtest[(vhHadTestY>0.5)], bins=50,weights=vhHadTestFW[(vhHadTestY>0.5)],range=[0,1], alpha=0.5, histtype='stepfilled',normed=1, color='red',label='EW qqH')
...
plt.legend()
plt.savefig('R0C.png',bbox_inches = 'tight')
...
plotVars = ['dipho_lead_ptoM','dipho_sublead_ptoM','dipho_mva', 'dijet_leadEta','dijet_subleadEta','dijet_LeadJPt','dijet_SubJPt','dijet_abs_dEta', 'dijet_Mjj', 'dijet_nj', 'cosThetaStar','dipho_mass','cos_dijet_dipho_dphi', 'dijet_dipho_dEta','dijet_centrality_gg']
...
plotVarsX=['lead photon pT/mgg', 'sublead photon pT/mgg', 'diphoton MVA score', 'lead jet eta', 'sublead jet eta', 'lead jet pT', 'sublead jet pT', 'dijet dEta', 'dijet Mjj', 'number of jets', 'oosThetaStar','diphoton invariant mass','dijet dipho cos phi', 'dijet dipho dEta','dijet centrality gg']

plotVarsR=[(0,5),(0,5),(-1,1), (-3,3),(-3,3),(0,800),(0,800),(0,6),(60,120),(0,8),(-1,1),(100,180),(-1,1),(-3,3),(0,600),(0,8),(0,1)]
...
def plot_variable(var='cosThetaStar', var_label = '$\cos\,\theta^*$', setrange=(-1,1) , plot_type = 'EW qqH vs ggH'):
  ...
  if (plot_type=='EW qqH vs ggH'):
      ...
      plt.hist(numpy_EWqqH, bins=50,weights=numpy_EWqqH_weight,histtype='step', normed=1, color = 'red',range=setrange, label = 'EW qqH',linewidth=2.0)
