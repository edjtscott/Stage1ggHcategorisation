...
BDTFW = trainTotal['weight'].values
...
print 'area under roc curve for training set_ggh = %1.5f'%(1- roc_auc_score(BDTTrainY_ggh_roc, BDTPredYtrain[:,0], sample_weight = BDTTrainTW) )
print 'area under roc curve for test set_ggh     = %1.5f'%(1- roc_auc_score(BDTTestY_ggh_roc, BDTPredYtest[:,0], sample_weight = BDTTestFW)  )
roc_auc_train_ggh = 1- roc_auc_score(BDTTrainY_ggh_roc, BDTPredYtrain[:,0], sample_weight = BDTTrainTW)
roc_auc_test_ggh = 1- roc_auc_score(BDTTestY_ggh_roc, BDTPredYtest[:,0], sample_weight = BDTTestFW) 

fpr, tpr, thresholds= roc_curve(BDTTrainY_ggh_roc, BDTPredYtrain[:,0], pos_label=0, sample_weight = BDTTrainTW)
...
print 'Training performance:auc'
print 'area under roc curve for training set_vbf = %1.5f'%(roc_auc_score(BDTTrainY_vbf_roc, BDTPredYtrain[:,1], sample_weight = BDTTrainTW) )
...
print 'Training performance:auc'
print 'area under roc curve for training set_bkg = %1.5f'%(roc_auc_score(BDTTrainY_bkg_roc_auc, BDTPredYtrain[:,2], sample_weight = BDTTrainTW) )
...
roc_auc_train_bkg = roc_auc_score(BDTTrainY_bkg_roc_auc, BDTPredYtrain[:,2], sample_weight = BDTTrainTW)
roc_auc_test_bkg = roc_auc_score(BDTTestY_bkg_roc_auc, BDTPredYtest[:,2],sample_weight = BDTTestFW)


fpr, tpr, thresholds = roc_curve(BDTTrainY_bkg_roc, BDTPredYtrain[:,2], pos_label=2,sample_weight = BDTTrainTW)
#roc_auc=auc(fpr,tpr)
print 'loaded train roc curve'
plt.figure()
...
plt.hist(BDTPredClassTrain[(BDTTrainY==0)],bins=[0,1,2,3], weights=BDTTrainTW[(BDTTrainY==0)], histtype='step',normed=1, color='red',label='ggh (train)')
plt.hist(BDTPredClassTrain[(BDTTrainY==1)],bins=[0,1,2,3], weights=BDTTrainTW[(BDTTrainY==1)], histtype='step',normed=1, color='green',label='vbf (train')
plt.hist(BDTPredClassTrain[(BDTTrainY==2)], bins=[0,1,2,3], weights=BDTTrainTW[(BDTTrainY==2)], histtype='step',normed=1, color='blue',label='bkg (train')
